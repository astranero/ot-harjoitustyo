...
import ui.registration_check_ui as checktool


class Register:
    def __init__(self, root, show_login_view, show_user_view):
        self._checktool = checktool.CheckUi()
        self._root = root
    ...
    def _fetch_date(self):
        ...
        self._choose_btn = tk.Button(
            self._new_win, text="Choose", command=self._get_date, font=self._font).grid(row=1, column=0, **self.padding, sticky="nsew")
        self._exit_btn = tk.Button(
            self._new_win, text="Exit", command=self._destroy_date_screen, font=self._font).grid(row=1, column=1, **self.padding, sticky="nsew")
    ...
    def _fetch_name(self):
        ...
        name_ent = tk.Entry(self._frame, textvariable=self._name_entry, font=self._font)
    ...
    def _fetch_surname(self):
        ...
        surname_ent = tk.Entry(self._frame, textvariable=self._surname_entry, font=self._font)
        surname_ent.grid(row=4, column=1, **self.padding, sticky="nsew")

    def _fetch_email(self):
        email_lbl = tk.Label(self._frame, text="Enter Email Address", font=self._font)
    ...
    def _fetch_passwords(self):
        password1_lbl = tk.Label(self._frame, text="Enter Password", font=self._font)
        ...
        password2_lbl = tk.Label(self._frame, text="Enter Password", font=self._font)
    ...
    def _fetch_gender(self):
        gender_lbl = tk.Label(self._frame, text="Select Gender", font=self._font)
    ...
    def _register_screen(self):
        ...
        self._space.grid(row=1, column=0, columnspan=2, **self.padding, sticky="nsew")
        self._fetch_name()
        self._fetch_surname()
        self._fetch_email()
    ...
    def _savedata(self):
        ...
        try:
            if self._checktool._name_check_handling(name):
                if self._checktool._name_check_handling(surname):
                    if self._checktool._email_check_handling(email):
                        if self._checktool._password_check_handling(password1, password2):
                            if self._checktool._birthdate_check_handling(birthdate):
                                if self._checktool._gender_check_handling(gender):
                                    if self._checktool._height_check_handling(height):
