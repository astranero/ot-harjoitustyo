from tkinter import Button, Label, OptionMenu, StringVar, Tk, Frame, Toplevel, constants
...
class CalculatorScreen:
    ...
    def _span_suitable_screen(self):
        ...
        if choosed_option == "I'll count with my Lean Body Mass":
            ...
            Button(self._toplevel, text="Go Back", command=lambda:[self.toplev_destroy(), self._initialize()]).grid(row=4, column=4)
    ...
    def _calculation_handle(self):
        calculate_mode_lbl = Label(self._frame, text="I want to calculate:" ).grid(row=0, column=0, sticky="nsew")
        calculation_options = ["Basal Metabolic Rate", "Total Daily Energy Expenditure"]
        self.calc_var = StringVar()
        self.calc_var.set("Basal Metabolic Rate")
        calculate_menu = OptionMenu(self._frame, self.calc_var, *calculation_options, command=lambda x: [self.config_calc_var()])
    ...
    def _mode_handle(self):
        ...
        question_lbl = Label(master=self._frame, text="Choose one of the following options to count your BMR/TDEE?")
        question_lbl.grid(row=3, column=0)
        options = ["I'll count with my Lean Body Mass", "I'll count with my Body fat %", "I'll count by estimating my Lean Body Mass"]
        menu = OptionMenu(self._frame, self._choosed_var, *options, command=lambda x: [self.toplev_destroy(),self.config_choosed_var(), self._span_suitable_screen()])
        menu.grid(row=4, column=0, sticky="nsew")
        Button(self._frame, text="Go back", command=lambda:[self._user_view(self._email, self._password)]).grid(row=5, column=0, sticky="nsew")
